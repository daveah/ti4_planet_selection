...
class Results:
    ...
    def allocate(self, num_players):
        for player in range(0, num_players):
            if not self.fill_player(player, self.get_unused_vector(),
    ...
    def fill_player(self, player_num, unused_tiles,
    ...
    def configure_rip(self, res_infls):
    ...
    def configure_w(self):
        for ii in range(0, 4):
            self.allocate_planet(wormhole[ii], ii)
    ...
    def configure_ab(self, abs):
        ...
        for ii in range(0, len(num_anoms)):
            for jj in range(0, num_anoms[ii]):
                self.allocate_planet(reds[0], ii)
                reds = reds[1:]
        ...
        for ii in range(0, len(num_total)):
            for jj in range(0, num_total[ii]):
                self.allocate_planet(remain[0], ii)
                remain = remain[1:]
        for rem in remain:
            self.allocate_planet(rem, -1)
    ...
    def configure_5_players(self):
        self.num_tiles = 6
        self.allocate_planet(lowest[random.randint(0, 1)], -1)
